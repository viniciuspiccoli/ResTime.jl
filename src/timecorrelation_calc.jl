...
#  end  
  ...
  ##### function that includes all calculations for MY systems using the slow method- 
  export restime_calculation 
  ...
  function restime_calculation(il::String, main_dir::String, conc::String, file::String, cutoff::Vector{Float64}, peak::String, cell::Int64)
    ...
    if length(il) == 10   
      ...
      # time-correlation calculation for anion1
      trajectory = ResTime.Trajectory("processed.xtc", protein, anion1, format="XTC")
      time_an1, prob_an1 = ResTime.correlation_cell(trajectory,cutoff[1], cutoff_cl=cell)
      #println("passou da traj anion 1 ")    

      # time-correlation calculation for anion2
      trajectory = ResTime.Trajectory("processed.xtc", protein, anion2, format="XTC")
      time_an2, prob_an2 = ResTime.correlation_cell(trajectory,cutoff[2], cutoff_cl=cell) 

      #println("passou da traj anion 2 ")

      # time-correlation calculation for cation
      trajectory = ResTime.Trajectory("processed.xtc", protein, cation, format="XTC")
      time_cat, prob_cat = ResTime.correlation_cell(trajectory,cutoff[3], cutoff_cl=cell)
      
      # time-correlation calculation for WATER
      trajectory = ResTime.Trajectory("processed.xtc", protein, water, format="XTC")
      time_wat, prob_wat = correlation_cell(trajectory,cutoff[4], cutoff_cl=cell)
    ...
    elseif length(il) == 9
      ...
      # time-correlation calculation for cation
      trajectory = ResTime.Trajectory("processed.xtc", protein, cation, format="XTC")
      time_cat, prob_cat = correlation_cell(trajectory,cutoff[3], cutoff_cl=cell)
    ...
    elseif length(il) == 7
      ...
      # time-correlation calculation for cation
      trajectory = ResTime.Trajectory("processed.xtc", protein, cation, format="XTC")
      time_cat, prob_cat = correlation_cell(trajectory,cutoff[2], cutoff_cl=cell)
      
      # time-correlation calculation for WATER
      trajectory = ResTime.Trajectory("processed.xtc", protein, water, format="XTC")
      time_wat, prob_wat = correlation_cell(trajectory,cutoff[3], cutoff_cl=cell)
